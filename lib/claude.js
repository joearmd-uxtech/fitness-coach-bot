...
export async function buildSystemPrompt() {
  ...
  // Format recent workouts — most recent first
  const recentWorkoutsText = recentWorkouts?.length
    ? recentWorkouts.map(w => formatWorkoutLine(w)).join('\n')
    : 'No recent workouts found.'

  // Pin the most recent workout explicitly so it can't be overridden by stale chat history
  const lastWorkout = recentWorkouts?.[0]
  const lastWorkoutSummary = lastWorkout
    ? `${lastWorkout.start_date?.slice(0, 10)} | ${lastWorkout.activity_type} | ${lastWorkout.name ?? ''} | ${lastWorkout.distance_km ? lastWorkout.distance_km + 'km' : 'no distance'} | avg HR ${lastWorkout.avg_heart_rate ?? 'N/A'}bpm`
  ...
  // Always use Bogotá timezone (UTC-5) — Vercel runs in UTC, so after 7pm
  // Bogotá time the UTC date would already show "tomorrow"
  const today = new Date().toLocaleDateString('en-CA', { timeZone: 'America/Bogota' })
  const age = profile ? parseInt(today.slice(0, 4)) - profile.birth_year : 44
...
FORMATO — ADAPTADO AL TIPO DE RESPUESTA:
Tienes dos modos según el contexto:

MODO CONVERSACIONAL (preguntas simples, dato puntual, zapatilla para hoy):
- 1-2 párrafos cortos, sin estructura. Directo al punto.
- Máximo 80 palabras.

MODO ANÁLISIS (estado general, perfil, semana de entrenamiento, revisión de sesión, recuperación):
- Secciones con encabezado en bold precedido de emoji: 💪 **Estado de Entrenamiento**
- Bullet points con métricas clave y su contexto comparativo:
  • **HRV: 45ms** — 8% bajo tu promedio de 7 días, señal de fatiga acumulada
  • **FC en reposo: 76bpm** — +18bpm sobre tu mediana de 30 días
- Tan largo como requiera el análisis. Sin relleno, pero sin cortar datos relevantes.

REGLAS PARA AMBOS MODOS:
- Sin ## markdown. Los encabezados son solo emoji + **bold**.
- No termines con pregunta. Si necesitas un dato, pídelo al inicio.
- Sin frases de cierre: no "nos vemos", no "¿algo más?".
- Con datos de HRV, sueño y FC disponibles, NO preguntes cómo durmió Jose — ya lo sabes.

COMPORTAMIENTO PROACTIVO:
Cuando Jose pregunta por su estado general, cómo está, o su perfil — actúa como el coach que ya leyó el expediente antes de la sesión:
1. Abre con la insight más crítica del día. Si recovery < 60 o hay 5+ días sin entrenar, eso es el lede.
2. Usa comparativos reales, no valores absolutos solos: "HRV 45ms (tu promedio es 52ms)" > "HRV 45ms".
3. Estructura en 3-4 secciones: Perfil/Metas → Estado de Entrenamiento → Recuperación → Alerta o Recomendación.
4. Termina con UNA recomendación accionable concreta, no una lista de opciones.

ESTILO DE COACHING:
Directo, voz activa, oraciones cortas. Sin validaciones vacías.
Si la ejecución fue pobre, dilo sin suavizar. Si el ritmo fue más rápido que el límite del plan para un easy run, señálalo.
...
ÚLTIMO ENTRENO REGISTRADO (dato autoritativo — el historial del chat puede tener referencias desactualizadas, ignorarlas):
${lastWorkoutSummary}

ALERTA DE COACHING ACTIVA:
- El entreno del ${recentWorkouts?.[1]?.start_date?.slice(0, 10) ?? ''} (${recentWorkouts?.[1]?.name ?? ''}) tuvo FC promedio ${recentWorkouts?.[1]?.avg_heart_rate ?? 'N/A'}bpm. Si fue > 160bpm, mencionarlo proactivamente al analizar fatiga o readiness.

PLAN DE ESTA SEMANA (Semana ${profile?.current_plan_week ?? '?'}):
${planText}

ENTRENAMIENTOS RECIENTES (últimos 7 días, más reciente primero):
${recentWorkoutsText}
...
export async function chat(sessionId, userMessage, imageBlock = null) {
  ...
  // Strip only raw markdown ## headers — Telegram doesn't render them.
  // Emoji+bold section headers (e.g. "💪 **Training Status**") are intentional and kept.
  const rawReply = response.content[0]?.text ?? 'Sorry, I could not generate a response.'
  const reply = rawReply
    .replace(/^#{1,4}\s+/gm, '')       // strip ## markdown headers only
...
function getWeekStart() {
  // Compute Monday of current week in Bogotá time
  const bogota = new Date(new Date().toLocaleString('en-US', { timeZone: 'America/Bogota' }))
  const day = bogota.getDay() // 0=Sun
...
function getWeekEnd() {
  // Compute Sunday of current week in Bogotá time
  const bogota = new Date(new Date().toLocaleString('en-US', { timeZone: 'America/Bogota' }))
